...
class AvisoPublicoView(APIView):
    """Aviso de privacidad para el socio que llega desde el link de WhatsApp.

    Sin login: a este punto el socio todavía no tiene cuenta, solo el QR que
    recepción le mandó. El token del QR (impredecible, ver `generar_token_qr`)
    es identificador suficiente para no exponer datos de otro socio.
    """

    permission_classes = [permissions.AllowAny]

    def _socio_de(self, token):
        ...
        return metodo.socio if metodo else None

    def get(self, request, token):
        socio = self._socio_de(token)
        if not socio:
            return Response({'error': 'Código no válido.'}, status=status.HTTP_404_NOT_FOUND)

        doc = DocumentoLegal.vigente(DocumentoLegal.AVISO_PRIVACIDAD, socio.gym_id)
        if not doc:
            return Response(
                {'error': 'Este gimnasio todavía no publica su aviso de privacidad.'},
                status=status.HTTP_404_NOT_FOUND,
            )

        ya_acepto = ConsentimientoSocio.objects.filter(socio=socio, documento=doc).exists()
        return Response({
            'socio_nombre': f'{socio.nombre} {socio.apellido}',
            'gym_nombre': socio.gym.nombre,
            'documento': DocumentoLegalSerializer(doc).data,
            'requiere_tutor': bool(socio.es_menor),
            'ya_acepto': ya_acepto,
            # El QR solo se revela una vez aceptado: es la razón de ser de la página.
            'codigo_acceso': token if ya_acepto else None,
        })


class AceptarAvisoPublicoView(APIView):
    """Registra la aceptación del socio y, solo entonces, entrega el código QR."""
    ...
    def post(self, request, token):
        ...
        es_menor = bool(socio.es_menor)
        ConsentimientoSocio.objects.get_or_create(
            socio=socio, documento=doc,
            defaults={
                'otorgado_por': 'tutor' if es_menor else 'socio',
                'medio': 'digital',
                'tutor_nombre': socio.tutor_nombre if es_menor else '',
                'tutor_parentesco': socio.tutor_parentesco if es_menor else '',
                'ip': ip_de(request),
            },
        )
        return Response({'codigo_acceso': token})
