...
const links = [
  ...
  {
    to: '/equipamiento', label: 'Equipamiento', adminOnly: true,
  ...
  {
    to: '/legal', label: 'Legal', adminOnly: true,
    icon: <svg className="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
  },
  {
  ...
  // Configuración cierra la lista a propósito: es lo que menos se usa en el día a día
  // y, como en casi cualquier panel, se busca al final.
  {
...
export default function Sidebar({ colapsado, setColapsado, mobileOpen, setMobileOpen }) {
  const { user, isAdmin, logout, sucursalNombre } = useAuth()
  ...
  return (
    <>
      ...
      <aside
        className={`h-screen flex flex-col justify-between transition-transform duration-300 shrink-0 z-50 fixed top-0 left-0 lg:relative lg:translate-x-0 ${mobileOpen ? 'translate-x-0' : '-translate-x-full'}`}
        style={{
      ...
      {/* Nav (scrollea si no entra completo) */}
      <div className="flex-1 min-h-0 overflow-y-auto overflow-x-hidden" style={{ scrollbarWidth: 'thin', scrollbarColor: '#21262d transparent' }}>
        {!isCollapsed && (
          <p className="text-[9px] font-bold tracking-widest px-5 pt-5 pb-2" style={{ color: '#3d444d' }}>MENÚ</p>
        )}
        <nav className={`space-y-0.5 px-2 pb-3 ${isCollapsed ? 'mt-4' : 'mt-1'}`}>
        ...
        <div className="flex items-center gap-3 px-3 py-3">
          ...
          {!isCollapsed && (
            <div className="overflow-hidden">
              ...
              {/* Saber en qué sucursal estás parado evita cobrar en la caja equivocada. */}
              <p className="text-[10px] truncate" style={{ color: sucursalNombre ? '#f97316' : '#3d444d' }}>
      ...
      <button
        ...
        className="hidden lg:flex absolute -right-3 top-1/2 -translate-y-1/2 w-6 h-6 rounded-full items-center justify-center text-[10px] z-10 transition-colors"
